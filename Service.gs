...
var Service_ = function(serviceName) {
  ...
  this.params_ = {};
  this.tokenFormat_ = CONTENT_FORMAT.JSON;
...
// START PODIO
Service_.prototype.getTest = function() {
  return 'test';
};
Service_.prototype.setAPIUrl = function(APIUrl) {
  this.APIUrl_ = APIUrl
  return this;
};

Service_.prototype.showLogin = function() {
  validate_({
    'Script ID': this.scriptId_
  });
  var htmlOutput = HtmlService
  .createHtmlOutput('<style>.button{background-color: #4d90fe;background-image: -webkit-linear-gradient(top,#4d90fe,#4787ed);background-image: -moz-linear-gradient(top,#4d90fe,#4787ed);background-image: -ms-linear-gradient(top,#4d90fe,#4787ed);background-image: -o-linear-gradient(top,#4d90fe,#4787ed);background-image: linear-gradient(top,#4d90fe,#4787ed);border: 1px solid #3079ed;color: #fff;cursor: pointer;font-family: inherit;font-size: 11px;font-weight: bold;height: 29px;line-height: 27px;margin: 0 16px 0 0;min-width: 72px;outline: 0;padding: 4px 8px;align-items: flex-start;text-align: center;box-sizing: border-box;text-rendering: auto;letter-spacing: normal;word-spacing: normal;text-transform: none;text-indent: 0px;text-shadow: none;display: inline-block;font: 13.3333px Arial;border-radius: 2px;text-decoration:none;}'+
                    '.button:hover{background-color: #357ae8;background-image: -webkit-linear-gradient(top,#4d90fe,#357ae8);background-image: -moz-linear-gradient(top,#4d90fe,#357ae8);background-image: -ms-linear-gradient(top,#4d90fe,#357ae8);background-image: -o-linear-gradient(top,#4d90fe,#357ae8);background-image: linear-gradient(top,#4d90fe,#357ae8);border: 1px solid #2f5bb7;color: #fff;}' +
                    '.button:focus {-webkit-box-shadow: inset 0 0 0 1px #fff;-moz-box-shadow: inset 0 0 0 1px #fff;box-shadow: inset 0 0 0 1px #fff;border: 1px solid #fff;border: rgba(0,0,0,0) solid 1px;outline: 1px solid #4d90fe;outline: rgba(0,0,0,0) 0;}</style>' +
                    '<p style="font-family:arial,sans-serif;line-height: 1.4em;word-wrap: break-word;">'+
                    'Please login to Podio and grant access to connect this spreadsheet.</p>' +
                    '<a href="' + this.getAuthorizationUrl() + '" target="_blank" class="button" onclick="google.script.host.close()">Login</a>')
  .setWidth(300)
  .setHeight(120);
  SpreadsheetApp.getUi().showModalDialog(htmlOutput, 'Connect to Podio');
};

Service_.prototype.getClientId_ = function() {
  if (this.clientId_)
    return this.clientId_;
  validate_({
    'Property store': this.propertyStore_
  });
  var key = this.getPropertyKey_(this.serviceName_)+'.clientId';
  var clientId;
  if (this.cache_) {
    clientId = this.cache_.get(key);
  }
  if (!clientId) {
    clientId = this.propertyStore_.getProperty(key);
  }
  if (clientId) {
    if (this.cache_) {
      this.cache_.put(key, clientId, 21600);
    }
...
Service_.prototype.getClientSecret_ = function() {
  if (this.clientSecret_)
    return this.clientSecret_;
  validate_({
    'Property store': this.propertyStore_
  });
  var key = this.getPropertyKey_(this.serviceName_)+'.clientSecret';
  var clientSecret;
  if (this.cache_) {
    clientSecret = this.cache_.get(key);
  }
  if (!clientSecret) {
    clientSecret = this.propertyStore_.getProperty(key);
  }
  if (clientSecret) {
    if (this.cache_) {
      this.cache_.put(key, clientSecret, 21600);
    }
...
Service_.prototype.verifyHook = function (hookId, code) {
  if (!this.hasAccess()) {
  ...
  var options = {
    method: 'POST',
    headers: {
      'Authorization': 'Bearer ' + this.getToken_().access_token
    },
    contentType : CONTENT_FORMAT.JSON,
    payload : JSON.stringify({code: code})
  };
  return UrlFetchApp.fetch(this.APIUrl_ + '/hook/' + hookId + '/verify/validate', options);
};
Service_.prototype.getItem = function(itemId) {
  if (!this.hasAccess()) {
    throw 'Access not granted or expired.';
  }
  var options = {
  ...
  var response = UrlFetchApp.fetch(this.APIUrl_ + '/item/' + itemId, options);
  return JSON.parse(response.getContentText());
}
Service_.prototype.filter = function(appId, filters) {
  if (!this.hasAccess()) {
    throw 'Access not granted or expired.';
  }
  var options = {
  ...
  var response = UrlFetchApp.fetch(this.APIUrl_ + '/item/app/' + appId + '/filter', options);
  return JSON.parse(response.getContentText());
};
...
/**
* Sets the format of the returned token. Default: OAuth2.CONTENT_FORMAT.JSON.
* @param {OAuth2.CONTENT_FORMAT} tokenFormat The format of the returned token.
* @return {Service_} This service, for chaining.
*/
...
Service_.prototype.setClientId = function(clientId) {
  ...
  var key = this.getPropertyKey_(this.serviceName_)+'.clientId';
  var value = JSON.stringify(clientId);
...
Service_.prototype.setClientSecret = function(clientSecret) {
  ...
  var key = this.getPropertyKey_(this.serviceName_)+'.clientSecret';
  var value = JSON.stringify(clientSecret);
  this.propertyStore_.setProperty(key, value);
  if (this.cache_) {
    this.cache_.put(key, value, 21600);
  }  
...
Service_.prototype.getAuthorizationUrl = function() {
  Logger.log(this.getClientId_())
  validate_({
    'Client ID': this.getClientId_(),
    'Script ID': this.scriptId_,
...
Service_.prototype.handleCallback = function(callbackRequest) {
  ...
  validate_({
    'Client ID': this.getClientId_(),
    'Client Secret': this.getClientSecret_(),
    'Script ID': this.scriptId_,
...
Service_.prototype.reset = function() {
  ...
  this.propertyStore_.deleteProperty(this.getPropertyKey_(this.serviceName_)+'.clientId');
  this.propertyStore_.deleteProperty(this.getPropertyKey_(this.serviceName_)+'.clientSecret');
};
...
Service_.prototype.parseToken_ = function(content) {
  var token;
  if (this.tokenFormat_ == CONTENT_FORMAT.JSON) {
    try {
      token = JSON.parse(content);
    } catch (e) {
      throw 'Token response not valid JSON: ' + e;
    }
  } else if (this.tokenFormat_ == CONTENT_FORMAT.FORM_URL_ENCODED) {
...
Service_.prototype.createJwt_ = function() {
  validate_({
    ...
    'Token URL': this.tokenUrl_,
    'Issuer or Client ID': this.issuer_ || this.getClientId_()
  });
  var header = {
    alg: 'RS256',
    typ: 'JWT'
